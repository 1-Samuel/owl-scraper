...
func main() {
	...
	for i := 0; i < 1; i++ {
		url := "https://pk0yccosw3.execute-api.us-east-2.amazonaws.com/production/v2/content-types/schedule/blt27f16f110b3363f7/week/" + strconv.Itoa(i) + "/team/allteams?locale=en-us"
		...
		for _, event := range response.Data.TableData.Events {
			for _, match := range event.Matches {
				println(match.Competitors[0].Name + " - " + match.Competitors[1].Name)
				teams := make([]Team, 2)
				for i := 0; i < 2; i++ {
					score := 0
				...
				_, err := collection.ReplaceOne(context.TODO(), filter, match, opts)
				if err != nil {
					panic(err)
				}
			}
